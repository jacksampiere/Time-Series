...
# Denmark
ts.de.50 = agg(df.de, 'de')$ts.50
# France
ts.fr.50 = agg(df.fr, 'fr')$ts.50
# England
ts.gb.50 = agg(df.gb, 'gb')$ts.50
# Netherlands
ts.nl.50 = agg(df.nl, 'nl')$ts.50
# Sweden
ts.se.50 = agg(df.se, 'se')$ts.50
# Australia
ts.au.50 = agg(df.au, 'au')$ts.50
# Brazil
ts.br.50 = agg(df.br, 'br')$ts.50
# Canada
ts.ca.50 = agg(df.ca, 'ca')$ts.50
# Mexico
ts.mx.50 = agg(df.mx, 'mx')$ts.50
# US
ts.us.50 = agg(df.us, 'us')$ts.50


# create time series for each bin
ts.50 = cbind(ts.de.50, ts.fr.50, ts.gb.50, ts.nl.50, ts.se.50, ts.au.50,
              ts.br.50, ts.ca.50, ts.mx.50, ts.us.50)
...
ts.var.1 = ts.50[,c('gb_50', 'ca_50', 'us_50', 'mx_50', 'br_50')]
ts.var.2 = ts.50[,c('de_50', 'nl_50', 'gb_50')]
...
lags.1 = c(1,2,7,8) # chosen based on ACF plots / tuning
...
plot(pred.var.1)
# extract predictions
pred.val.gb = pred.var.1$fcst$gb_50[1:14]
pred.val.mx = pred.var.1$fcst$mx_50[1:14]
pred.val.ca = pred.var.1$fcst$ca_50[1:14]
pred.val.br = pred.var.1$fcst$br_50[1:14]
pred.val.us = pred.var.1$fcst$us_50[1:14]

# evaluate validation set performance
rmse.gb = rmse(val.var.1$gb_50, pred.val.gb)
mape.gb = mape(val.var.1$gb_50, pred.val.gb)
print(rmse.gb)
print(mape.gb*100)

rmse.mx = rmse(val.var.1$mx_50, pred.val.mx)
mape.mx = mape(val.var.1$mx_50, pred.val.mx)
print(rmse.mx)
print(mape.mx*100)

rmse.ca = rmse(val.var.1$ca_50, pred.val.ca)
mape.ca = mape(val.var.1$ca_50, pred.val.ca)
print(rmse.ca)
print(mape.ca*100)

rmse.br = rmse(val.var.1$br_50, pred.val.br)
mape.br = mape(val.var.1$br_50, pred.val.br)
print(rmse.br)
print(mape.br*100)

rmse.us = rmse(val.var.1$us_50, pred.val.us)
mape.us = mape(val.var.1$us_50, pred.val.us)
print(rmse.us)
print(mape.us*100)
...
gb = val.var.1[,c('gb_50','gb_pred')]
mx = val.var.1[,c('mx_50','mx_pred')]
ca = val.var.1[,c('ca_50','ca_pred')]
br = val.var.1[,c('br_50','br_pred')]
us = val.var.1[,c('us_50','us_pred')]
# plot forecasts
ggplot(gb, aes(index(gb))) +
  geom_line(aes(y=gb_50, color="True Values"), size=1.5) +
  geom_line(aes(y=gb_pred, color="Forecast"), size=1.5) + 
  ggtitle("Forecast of Stream Volume of Top 50 Tracks, Great Britain") +
  xlab('Date') + ylab('Streams') + 
  theme(plot.title = element_text(hjust = 0.5), legend.title=element_blank()) +
  scale_x_continuous(breaks=index(gb)[c(T,F)])

ggplot(mx, aes(index(mx))) +
  geom_line(aes(y=mx_50, color="True Values"), size=1.5) +
  geom_line(aes(y=mx_pred, color="Forecast"), size=1.5) + 
  ggtitle("Forecast of Stream Volume of Top 50 Tracks, Mexico") +
  xlab('Date') + ylab('Streams') + 
  theme(plot.title = element_text(hjust = 0.5), legend.title=element_blank()) +
  scale_x_continuous(breaks=index(mx)[c(T,F)])

ggplot(ca, aes(index(ca))) +
  geom_line(aes(y=ca_50, color="True Values"), size=1.5) +
  geom_line(aes(y=ca_pred, color="Forecast"), size=1.5) + 
  ggtitle("Forecast of Stream Volume of Top 50 Tracks, Canada") +
  xlab('Date') + ylab('Streams') + 
  theme(plot.title = element_text(hjust = 0.5), legend.title=element_blank()) + 
  scale_x_continuous(breaks=index(ca)[c(T,F)])

ggplot(br, aes(index(br))) +
  geom_line(aes(y=br_50, color="True Values"), size=1.5) +
  geom_line(aes(y=br_pred, color="Forecast"), size=1.5) + 
  ggtitle("Forecast of Stream Volume of Top 50 Tracks, Brazil") +
  xlab('Date') + ylab('Streams') + 
  theme(plot.title = element_text(hjust = 0.5), legend.title=element_blank()) + 
  scale_x_continuous(breaks=index(br)[c(T,F)])

ggplot(us, aes(index(us))) +
  geom_line(aes(y=us_50, color="True Values"), size=1.5) +
  geom_line(aes(y=us_pred, color="Forecast"), size=1.5) + 
  ggtitle("Forecast of Stream Volume of Top 50 Tracks, US") +
  xlab('Date') + ylab('Streams') + 
  theme(plot.title = element_text(hjust = 0.5), legend.title=element_blank()) + 
...
plot(pred.var.1)
# extract predictions
pred.test.gb = pred.var.1$fcst$gb_50[1:14]
pred.test.mx = pred.var.1$fcst$mx_50[1:14]
pred.test.ca = pred.var.1$fcst$ca_50[1:14]
pred.test.br = pred.var.1$fcst$br_50[1:14]
pred.test.us = pred.var.1$fcst$us_50[1:14]

# evaluate test set performance
rmse.gb = rmse(test.var.1$gb_50, pred.test.gb)
mape.gb = mape(test.var.1$gb_50, pred.test.gb)
print(rmse.gb)
print(mape.gb*100)

rmse.mx = rmse(test.var.1$mx_50, pred.test.mx)
mape.mx = mape(test.var.1$mx_50, pred.test.mx)
print(rmse.mx)
print(mape.mx*100)

rmse.ca = rmse(test.var.1$ca_50, pred.test.ca)
mape.ca = mape(test.var.1$ca_50, pred.test.ca)
print(rmse.ca)
print(mape.ca*100)

rmse.br = rmse(test.var.1$br_50, pred.test.br)
mape.br = mape(test.var.1$br_50, pred.test.br)
print(rmse.br)
print(mape.br*100)

rmse.us = rmse(test.var.1$us_50, pred.test.us)
mape.us = mape(test.var.1$us_50, pred.test.us)
print(rmse.us)
print(mape.us*100)
...
# subset test data
gb = test.var.1[,c('gb_50','gb_pred')]
mx = test.var.1[,c('mx_50','mx_pred')]
ca = test.var.1[,c('ca_50','ca_pred')]
br = test.var.1[,c('br_50','br_pred')]
us = test.var.1[,c('us_50','us_pred')]
# plot forecasts
ggplot(gb, aes(index(gb))) +
  geom_line(aes(y=gb_50, color="True Values"), size=1.5) +
  geom_line(aes(y=gb_pred, color="Forecast"), size=1.5) + 
  ggtitle("Great Britain Top 50") +
  xlab('Date') + ylab('Streams') + 
...
n.vars.2 = 3
# specify which lags to use (tune with validation set)
lags.2 = c(1,2,7,8,14,15) # chosen based on ACF plots / tuning
# length of lag vector for each regressor (add 1 for constant)
...
plot(pred.var.2)
# extract predictions
pred.val.de = pred.var.2$fcst$gb_50[1:14]
pred.val.nl = pred.var.2$fcst$nl_50[1:14]

# evaluate validation set performance
rmse.de = rmse(val.var.2$de_50, pred.val.de)
mape.de = mape(val.var.2$de_50, pred.val.de)
print(rmse.de)
print(mape.de*100)

rmse.nl = rmse(val.var.2$nl_50, pred.val.nl)
mape.nl = mape(val.var.2$nl_50, pred.val.nl)
print(rmse.nl)
print(mape.nl*100)
...
# subset validation data
de = val.var.2[,c('de_50','de_pred')]
nl = val.var.2[,c('nl_50','nl_pred')]
# plot forecasts
ggplot(de, aes(index(de))) +
  geom_line(aes(y=de_50, color="True Values"), size=1.5) +
  geom_line(aes(y=de_pred, color="Forecast"), size=1.5) + 
  ggtitle("Forecast of Stream Volume of Top 50 Tracks, Denmark") +
  xlab('Date') + ylab('Streams') + 
  theme(plot.title = element_text(hjust = 0.5), legend.title=element_blank()) +
  scale_x_continuous(breaks=index(de)[c(T,F)])

ggplot(nl, aes(index(nl))) +
  geom_line(aes(y=nl_50, color="True Values"), size=1.5) +
  geom_line(aes(y=nl_pred, color="Forecast"), size=1.5) + 
  ggtitle("Forecast of Stream Volume of Top 50 Tracks, Netherlands") +
  xlab('Date') + ylab('Streams') + 
  theme(plot.title = element_text(hjust = 0.5), legend.title=element_blank()) +
...
plot(pred.var.2)
# extract predictions
pred.test.de = pred.var.2$fcst$de_50[1:14]
pred.test.nl = pred.var.2$fcst$nl_50[1:14]

# evaluate test set performance
rmse.de = rmse(test.var.2$de_50, pred.test.de)
mape.de = mape(test.var.2$de_50, pred.test.de)
print(rmse.de)
print(mape.de*100)

rmse.nl = rmse(test.var.2$nl_50, pred.test.nl)
mape.nl = mape(test.var.2$nl_50, pred.test.nl)
print(rmse.nl)
print(mape.nl*100)
...
# subset test data
de = test.var.2[,c('de_50','de_pred')]
nl = test.var.2[,c('nl_50','nl_pred')]
# plot forecasts
ggplot(de, aes(index(de))) +
  geom_line(aes(y=de_50, color="True Values"), size=1.5) +
  geom_line(aes(y=de_pred, color="Forecast"), size=1.5) + 
  ggtitle("Denmark Top 50") +
  xlab('Date') + ylab('Streams') + 
  theme(plot.title = element_text(hjust = 0.5, size=20), legend.title=element_blank()) +
  scale_x_continuous(breaks=index(de)[c(T,F)])
